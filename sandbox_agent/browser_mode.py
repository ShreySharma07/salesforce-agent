...
from sandbox_agent.packs import AppPack, get_pack
from sandbox_agent.packs.salesforce.ui import (
    _cancel_advanced_search_modal,
    _is_escalation_row,
    _is_global_search_bar,
    _lookup_pill_has_value,
    _poll_for_inline_option,
    _poll_for_option,
    _verify_lookup_pill,
    _wait_for_lwc_panel,
    _wait_for_salesforce_ready,
)
from sandbox_agent.schemas import LoopIteration

log = logging.getLogger("browser-react")

# The generic agent prompt. An app pack contributes extra rule bullets that
# are spliced in at the end of the "Rules:" list (see build_system_prompt).
CORE_PROMPT_HEAD = """You are a browser automation agent running a Reason-Act-Observe loop. You are given ONE goal and must accomplish it on a live web page by choosing one action at a time.
...
# Active app pack
# ---------------------------------------------------------------------------
# One run = one plan = one pack (a container runs a single plan), so the pack
# is module state set once by the executor, like the navigation allowlist.
_active_pack: AppPack = get_pack(None)


def set_active_pack(pack: AppPack) -> None:
    global _active_pack
    _active_pack = pack


def active_pack() -> AppPack:
    return _active_pack


def build_system_prompt(pack: AppPack) -> str:
    """Core rules + the pack's rule bullets, spliced into the "Rules:" list."""
    rules = f"\n{pack.prompt_rules}" if pack.prompt_rules else ""
    return f"{CORE_PROMPT_HEAD}{rules}{CORE_PROMPT_TAIL}"


def _system_prompt() -> str:
    return build_system_prompt(_active_pack)
...
# hosts the active app pack legitimately moves between (e.g. Lightning's
# *.force.com / *.salesforce.com redirects).
...
def navigation_blocked_reason(url: str) -> str | None:
    ...
    if host in _allowed_nav_hosts or _active_pack.owns_host(host):
        return None
...
def wait_for_stable(page: Page, *, settle_ms: int = 400) -> None:
    ...
    try:
        ...
        pass
    # Wait for the app's own loading indicators (e.g. Lightning spinners).
    _active_pack.settle(page, timeout=2.0)
...
def execute_step(
    ...
    *,
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        if iteration == 1:
            _active_pack.settle(page, timeout=1.5)
            time.sleep(0.2)
        else:
            wait_for_stable(page)
        _active_pack.before_observe(page)   # app hooks, e.g. hide toasts BEFORE grounding
        url = page.url
        try:
            title = page.title() or ""
        except Exception:
        ...
        if action is None:
            ...
            try:
                ...
                response = llm.generate(
                    prompt=retry_prompt, system=_system_prompt(),
        ...
        # set, auto-succeed without another LLM round.  See RULE 2 in CORE_PROMPT_TAIL.
