...
The prompts ARE the product here: the plan-synthesis prompt encodes every
hard-won rule about how Salesforce behaves, and the captioner prompt decides
how faithfully a recording is described. Keeping them beside the code that
happens to call them made them invisible and hard to diff; this package is
where they live so a prompt change reads as a prompt change in review.

Exports:
  PLAN_SYSTEM_PROMPT      captions timeline -> structured Plan JSON
  CAPTION_SYSTEM_PROMPT   keyframe batch    -> one description per frame
  JSON_ONLY_RULE          the shared "no prose, no fences" contract
"""
from __future__ import annotations

# Both prompts demand raw JSON. Stated once so the two cannot drift apart.
JSON_ONLY_RULE = (
    "Output ONLY the JSON value described above. No prose, no explanation, "
...
__all__ = ["PLAN_SYSTEM_PROMPT", "CAPTION_SYSTEM_PROMPT", "JSON_ONLY_RULE"]
